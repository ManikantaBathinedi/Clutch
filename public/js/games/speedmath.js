// Speed Math — Client-side game logic
(function () {
  ...
  function renderProblem(data) {
    hasAnswered = false;
    pendingResult = null;
    if (window.updateGameStatus) window.updateGameStatus(`Problem ${data.problemNumber}/${data.totalProblems}`);

    gameView.innerHTML = `
      <div class="fade-in" style="width: 100%; max-width: 640px; margin: 0 auto;">
        <div class="question-number">
          Problem ${data.problemNumber} / ${data.totalProblems}
        </div>
        ...
        <div class="question-text" style="font-size: 3.2rem; font-family: var(--font-display); letter-spacing: 6px; margin: 24px 0;">${escapeHtml(data.equation)} = ?</div>

        <div style="max-width: 320px; margin: 0 auto;">
          <input type="number" id="math-input" class="game-input" placeholder="Answer" autocomplete="off"
            style="text-align: center; font-size: 1.5rem; font-family: var(--font-display); font-weight: 800;">
          <button class="btn btn-sm btn-primary mt-12" id="submit-math">Submit</button>
        </div>
  ...
  function disableInput() {
    const input = document.getElementById('math-input');
    const btn = document.getElementById('submit-math');
    ...
    gameView.innerHTML = `
      <div class="results-container fade-in" style="width: 100%;">
        <div class="result-header">
          ...
          <h2>${escapeHtml(data.equation)} = <span style="color: var(--success);">${escapeHtml(data.correctAnswer)}</span></h2>
        </div>
        <ul class="leaderboard" id="round-leaderboard"></ul>
        ${isHost ? `
          <button class="btn btn-sm btn-primary mt-20" id="next-btn">
            ${data.problemNumber < data.totalProblems ? 'Next Problem' : 'Final Results'}
          </button>
        ` : `
  ...
  socket.on('game-state', renderProblem);
